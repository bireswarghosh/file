...
const PackageManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.services.forEach((service, index) => {
        formDataToSend.append(`services[${index}][serviceName]`, service.serviceName);
        formDataToSend.append(`services[${index}][serviceDescription]`, service.serviceDescription || '');
      });
      
      if (selectedImage) {
        formDataToSend.append('packageImage', selectedImage);
      }
  ...
  const handleEdit = (pkg) => {
    ...
    setImagePreview(pkg.packageImage ? `https://xrk77z9r-5000.inc1.devtunnels.ms/uploads/packages/${pkg.packageImage}` : null);
    setShowModal(true);
  };
                      ...
                      {packages.map((pkg) => (
                        <tr key={pkg.id}>
                          <td>
                            {pkg.packageImage ? (
                              <img 
                                src={`https://xrk77z9r-5000.inc1.devtunnels.ms/uploads/packages/${pkg.packageImage}`} 
                                alt={pkg.packageName}
                                style={{width: '50px', height: '50px', objectFit: 'cover', borderRadius: '8px'}}
                              />
                            ) : (
                              <div 
                                style={{width: '50px', height: '50px', backgroundColor: '#f8f9fa', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center'}}
                              >
