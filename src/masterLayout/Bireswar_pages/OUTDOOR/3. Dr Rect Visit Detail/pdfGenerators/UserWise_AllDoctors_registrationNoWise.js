...
export const generateUserWiseAllDoctorsRegistrationNoWisePDF = (data, fromDate, toDate) => {
  ...
  // doc.setFontSize(8);
  // doc.setFont("helvetica", "normal");
  // doc.text(`Date : ${fromDate}`, 10, currentY);
  // doc.text(`To : ${toDate}`, 80, currentY);
  // currentY += 5;
  ...
  const visitData = data.map(visit => {
    ...
    return [
      { content: `Regn No : ${regNo}`, colSpan: 10, styles: { fontStyle: 'bold', halign: 'left', fillColor: [240, 240, 240] } }
    ];
  });
  
  // Add registration number headers and visit details
  data.forEach(visit => {
    const regNo = visit.RegistrationNo || visit.RegNo || `S-${visit.VNo}/23-24`;
    const doctorCh = visit.Rate || 0;
    const scCharge = visit.ServiceCharge || 0;
