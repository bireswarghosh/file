...
export const generateUserWiseAllDoctorsOnlyServiceChPDF = (data, fromDate, toDate) => {
  ...
  const addHeader = (pageNum) => {
    ...
    doc.setFontSize(11);
    doc.text("DATE WISE DOCTOR WISE VISIT (SERVICE CHARGE DETAIL)", pageWidth / 2, 30, { align: "center" });
    ...
    autoTable(doc, {
      head: [['Visit No', 'Patient Name', 'SRV Charge', 'Amount', 'Entry By']],
      body: [],
      startY: 45,
      theme: 'grid',
      headStyles: {
  ...
  Object.entries(groupedByDoctor).forEach(([doctorName, visits]) => {
    ...
    const doctorTotalSrvCharge = visits.reduce((sum, v) => sum + (v.ServiceCh || 0), 0);
    const doctorTotalAmount = visits.reduce((sum, v) => sum + (v.TotAmount || 0), 0);
    
    grandTotalSrvCharge += doctorTotalSrvCharge;
  ...
  autoTable(doc, {
    body: [[
      ...
      { content: grandTotalSrvCharge.toFixed(2), styles: { fontStyle: 'bold', halign: 'center', fillColor: [255, 255, 255] } },
