...
const [formData, setFormData] = useState({
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        setPagination({
          ...
          totalPages: paginationData.totalPages || Math.ceil((response.data.total || 0) / 20)
        });
  ...
  const fetchDoctorRates = async (doctorId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axiosInstance.get(`/doctormaster/${doctorId}/rates`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (response.data && response.data.success) {
      // Save doctor rates
      const doctorId = modalMode === 'edit' ? selectedDoctor.DoctorId : response.data.data.doctorId;
                        ...
                        <tbody>
                          {doctorList.map(doctor => (
                            <tr key={doctor.DoctorId}>
                              ...
                              <td>
                                <Button
                                  variant={doctor.Status === 'on' ? 'success' : 'danger'}
                                  size="sm"
                                  onClick={() => handleStatusToggle(doctor.DoctorId)}
                                >
                                  {doctor.Status === 'on' ? 'Active' : 'Inactive'}
                                </Button>
                              </td>
                              <td>
                                <div className="d-flex gap-2">
          ...
          <Row className="g-3">
            ...
            <Col md={12}>
              ...
              {visitTypes.map(visitType => {
                const existingRate = doctorRates.find(r => r.VisitTypeId === visitType.VisitTypeId);
                
                return (
                  <div key={visitType.VisitTypeId} className="mb-3 p-3 border rounded">
                    <h6 className="mb-3 text-primary">{visitType.VisitType}</h6>
                    <Row className="g-2">
                      <Col md={2}>
                        <Form.Group>
                          ...
                          <Form.Control 
                            ...
                            onChange={(e) => updateRate(visitType.VisitTypeId, 'Rate', e.target.value)}
                          />
                        </Form.Group>
                      </Col>
                      <Col md={2}>
                        <Form.Group>
                          ...
                          <Form.Control 
                            ...
                            onChange={(e) => updateRate(visitType.VisitTypeId, 'ServiceCh', e.target.value)}
                          />
                        </Form.Group>
                      </Col>
                      <Col md={2}>
                        <Form.Group>
                          ...
                          <Form.Control 
                            ...
                            onChange={(e) => updateRate(visitType.VisitTypeId, 'GroupA', e.target.value)}
                          />
                        </Form.Group>
                      </Col>
                      <Col md={2}>
                        <Form.Group>
                          ...
                          <Form.Control 
                            ...
                            onChange={(e) => updateRate(visitType.VisitTypeId, 'GroupB', e.target.value)}
                          />
                        </Form.Group>
                      </Col>
                      <Col md={2}>
                        <Form.Group>
                          ...
                          <Form.Control 
                            ...
                            onChange={(e) => updateRate(visitType.VisitTypeId, 'GroupC', e.target.value)}
                          />
                        </Form.Group>
                      </Col>
                      <Col md={2}>
                        <Form.Group>
                          ...
                          <Form.Control 
                            ...
                            onChange={(e) => updateRate(visitType.VisitTypeId, 'GroupD', e.target.value)}
                          />
                        </Form.Group>
                      </Col>
                    </Row>
