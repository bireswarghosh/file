...
const DataTable = (props) => {
  ...
  // Helper to get prefix/suffix for API endpoints
function getAdmissionPrefixSuffix(admission) {
  const prefix = admission.AdmitionId.toString().padStart(6, '0');
  const date = new Date(admission.AdmitionDate);
  const startYear = date.getFullYear().toString().slice(-2);
  const endYear = (date.getFullYear() + 1).toString().slice(-2);
  const suffix = `${startYear}-${endYear}`;
  return { prefix, suffix };
}

// Always use /admission/:prefix/:suffix and never append the suffix twice
const handleDelete = async (admission) => {
  if (window.confirm('Are you sure you want to delete this record?')) {
    try {
      const token = localStorage.getItem('token');
      // Only one suffix in the URL!
      const { prefix, suffix } = getAdmissionPrefixSuffix(admission);
      await axiosInstance.delete(`/admission/${prefix}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
    } catch (error) {
...
const handleSubmit = async (e) => {
  ...
  try {
    const token = localStorage.getItem('token');
    const { prefix, suffix } = getAdmissionPrefixSuffix(selectedAdmission);
    ...
    Object.keys(updatableFormData).forEach(key => {
      if (updatableFormData[key] === null || updatableFormData[key] === undefined) {
        delete updatableFormData[key];
      }
    });

    // Convert fields that must be integers if present and not empty
    const intFields = [
      'AreaId', 'BedId', 'UCDoctor2Id', 'UCDoctor3Id', 'DiseaseId', 'RMOId',
      'RefDoctorId', 'DietChartId', 'tpaper', 'RefDoctorId2'
    ];
    intFields.forEach(field => {
      if (updatableFormData[field] !== undefined && updatableFormData[field] !== "") {
        updatableFormData[field] = parseInt(updatableFormData[field], 10);
        if (isNaN(updatableFormData[field])) delete updatableFormData[field];
      }
    });
