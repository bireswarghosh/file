...
const Visit_list = () => {
  ...
  const fetchVisits = async (phone = '', date = '', page = 1, limit = 100) => {
    setLoading(true);
    try {
      const token = localStorage.getItem('token');
      let url = `/patient-with-bills?page=${page}&limit=${limit}`;
  ...
  const handleSearch = () => {
    ...
    fetchVisits(searchPhone, searchDate, 1, paginationModel.pageSize);
  };

  const handlePaginationChange = (newPaginationModel) => {
    setPaginationModel(newPaginationModel);
    fetchVisits(searchPhone, searchDate, newPaginationModel.page + 1, newPaginationModel.pageSize);
  };
  ...
  const handleUpdatePatient = async () => {
    try {
      ...
      if (response.data?.success) {
        ...
        fetchVisits(searchPhone, searchDate, paginationModel.page + 1, paginationModel.pageSize);
      }
    } catch (error) {
