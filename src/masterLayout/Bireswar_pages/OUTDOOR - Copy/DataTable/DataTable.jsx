...
const DataTable = (props) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axiosInstance.get('/api/v1/patientregistration?page=1&limit=50', {
        headers: { 
...
const handleDelete = async (admission) => {
  if (window.confirm('Are you sure you want to delete this record?')) {
    try {
      const token = localStorage.getItem('token');
      await axiosInstance.delete(`/api/v1/patientregistration/${admission.RegistrationId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
...
const handleEdit = async (admission) => {
  try {
    ...
    const response = await axiosInstance.get(
      `/api/v1/patientregistration/${admission.RegistrationId}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (response.data?.success) {
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const response = await axiosInstance.put(
      `/api/v1/patientregistration/${selectedAdmission.RegistrationId}`,
      formData,
      { headers: { 'Authorization': `Bearer ${token}` } }
    );
